/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package dao;

/**
 *
 * @author Famille Francois
 */

import connexion.Connexion;
import java.sql.ResultSet;
import java.util.ArrayList;
import java.util.List;
import modele.Ecole;

/**
 * classe DAO qui permet d'accéder à la table : <b>ecole</b>. Cette classe 
 * associe un objet modèle Ecole à la table ecole.<br> 
 * <br>
 * Un objet de cette classe permet de réaliser les opérations suivantes  :<br>
 * - Insérer un nouveau objet dans la base de données<br>
 * - Suppriemr un objet dans la base de données<br>
 * - Mettre à jour un objet dans la base de données <br>
 * - Avoir la liste des objets qui sont enregistrés dans la base de données
 * - Avoir la liste des objets qui vérifient un ensemble de critères
 * 
 * @author Famille Francois
 * 
 * @param <Ecole> : Ecole est une classe du package modele (Classes Modèles)
 */
public class DAOEcole extends DAO<Ecole> {
    /**
     * Constructeur de la classe
     * @param connexion : Le module de connexion qui permet d'accéder à la base de données
     */
    public DAOEcole(Connexion connexion) {
      // Créer une instance de DAOEcole avec la connexion et le nom de table : trimestre
      super(connexion, "ecole");
    }
    
    /**
    * Cette méthode permet de retourner une instance de la classe modèle Ecole à partir d'une ligne de ResultSet
    * 
    * @param data : un result set obtenu à partir d'une requête SQL Select
    * @return : une instance de type Ecole
    */
    @Override
    public Ecole getObjectFromData(ResultSet data){
        Ecole ecole = null;
        
        try{
            ecole = new Ecole();
            
            ecole.setId(data.getInt("id"));
            ecole.setNomEcole(data.getString("nom_ecole") );
            ecole.setAdresse(data.getString("adresse") );
        }
        catch(Exception e){
        }
        
        return ecole;
    }

    /**
    * Méthode d'insertion d'un objet du modèle Ecole dans la base de données (table ecole)
    * @param obj : l'objet à insérer
    * @return boolean : true si l'insertion a été exécutée avec succès, false sinon
    */
    @Override
    public boolean insert(Ecole obj) {
        String sql = "INSERT INTO `"+this.tableName+"` (`id`, `nom_ecole`, `adresse`) VALUES (NULL, '"+obj.getNomEcole()+"', '"+obj.getAdresse()+"')";
        boolean result = this.connexion.sendQuery(sql);
        
        try{
            ResultSet data = connexion.sendQuerySelect("select id from `"+this.tableName+"` order by id desc limit 1");
            if (data.next()){
                int id = data.getInt("id");
                obj.setId(id);
            }
        }
        catch(Exception e){
            e.printStackTrace();
        }
        
        return result;
    }

    /**
    * Méthode pour supprimer le tuple correspondant à l'objet du modèle Ecole de la base de données (table ecole)
    * @param obj : l'objet à supprimer
    * @return boolean : true si la suppression a été exécutée avec succès, false sinon
    */
    @Override
    public boolean delete(Ecole obj) {
        String sql = "DELETE FROM `"+this.tableName+"` WHERE `"+this.tableName+"`.`id` = "+obj.getId();
        
        return this.connexion.sendQuery(sql);
    }

    /**
    * Méthode de mise à jour l'objet d'un objet du modèle AnneeScoalire dans la base de données (table annee_scoalire)
    * @param obj : l'objet à mettre à jour
    * @return boolean : true si la mise à jour a été exécutée avec succès, false sinon
    */
    @Override
    public boolean update(Ecole obj) {
        String sql = "UPDATE `"+this.tableName+"` SET `nom_ecole` = '"+obj.getNomEcole()+"', `adresse` = '"+obj.getAdresse()+"' WHERE `"+this.tableName+"`.`id` = "+obj.getId();
        
        return this.connexion.sendQuery(sql);
    }

    /**
    * Méthode qui permet de rechercher une instance de la classe Modèle AnneeScoalire à partir de son id
    * @param id : l'identifiant à rechercher
    * @return Ecole : Une instance de la classe Ecole de l'objet à récupérer. Résultat null si id ne se trouve.
    */
    @Override
    public Ecole findById(int id) {
        Ecole ecole = null;
        
        String sql = "SELECT * FROM `"+this.tableName+"` WHERE `"+this.tableName+"`.id = "+id+" LIMIT 1";
        
        ResultSet data = this.connexion.sendQuerySelect(sql);
        
        try {
            if (data.next()){
                ecole = getObjectFromData(data);
            }
        } catch (Exception e) {
        }

        return ecole;
    }

    
    /**
    * Méthode qui permet de récupérer les objets qui vérifient les critères dans la base de doonées du modèle T (table ecole)
    * @return : Liste des objets dans le modèle Ecole qui vérfient les critères
    */
    @Override
    public List<Ecole> get(String criteres) {
        List<Ecole> list = new ArrayList<>();
        
        if (criteres.trim().equals("")){
            criteres = "1";
        }
        
        String sql = "SELECT * FROM `"+this.tableName+"` WHERE "+criteres;
        
        ResultSet data = this.connexion.sendQuerySelect(sql);
        
        try {
            while (data.next()){
               list.add( getObjectFromData(data) );
            }
        } catch (Exception e) {
        }
        
        return list;
    }
}
